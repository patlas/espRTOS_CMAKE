...
#define spi_cs_low(card)  do { GPIO_OUTPUT_SET(card->cs_pin, 0); } while(0)
#define spi_cs_high(card) do { GPIO_OUTPUT_SET(card->cs_pin, 1); } while(0)
#define spi_read_byte()   (spi_rxtx8(HSPI, 0xff))
...
{
    return (spi_rxtx8(HSPI, word >> 8) << 8) | spi_rxtx8(HSPI, word);
}

inline static void spi_read_bytes(uint8_t *dst, size_t size)
{	
	uint8_t *offs = dst;
    for (; offs < dst + size; offs ++)
        *offs = spi_read_byte();
}

static bool wait()
{
    uint32_t stop = system_get_time() + IO_TIMEOUT_US;
    while (spi_read_byte() != 0xff)
        if (system_get_time() >= stop)
            return false;
    return true;
}
...
{
    uint32_t timeout = system_get_time() + IO_TIMEOUT_US;

    while (true)
    {
        if (system_get_time() >= timeout)
...
sdio_error_t sdio_init(sdio_card_t *card, uint8_t cs_pin, uint32_t high_freq_divider)
{
   ...
   /* spi_settings_t s = {
    ...
    FCLK_SLOW();
    GPIO_AS_OUTPUT(card->cs_pin);

    uint32_t timeout = system_get_time() + INIT_TIMEOUT_US;
...
sdio_error_t sdio_write_sectors(sdio_card_t *card, uint32_t sector, uint8_t *src, uint32_t count)
{
    ...
    spi_rxtx8(HSPI, TOKEN_STOP_TRAN);
